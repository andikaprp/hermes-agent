...
def classify_fast_path(
    ...
    if not stripped:
        return None
    # Production may render every inbound user message with a leading timestamp
    # (``gateway.message_timestamps.enabled``), so what arrives here is
    # ``[Thu 2026-09-17 21:54:28 WIB] hi``. That leading ``[`` is indistinguishable from a
    # machine work trigger below — without stripping, *every* live message classifies as
    # not-ordinary while clean-string unit tests still pass (LAB-52 / LAB-3 live miss).
    # Judge the user's text, not the render: strip only the known timestamp shape.
