...
| `message_reactions` | `react_to_message` | One emoji reaction on the user's own message. Folded in per session: a chat platform whose adapter implements the reaction API (e.g. Telegram), and desktop sessions (which also list the tool under `desktop_ui`). Not part of any platform bundle. |
