...
| `TELEGRAM_REACTION_STYLE` | Kind of reactions to post when reactions are enabled: `receipt` (default) posts the automated 👀 then 👍/👎 lifecycle receipts, `content` posts none (the agent reacts deliberately via `send_message`), `off` equals disabling reactions. Equivalent to `telegram.reaction_style`. |
