...
| `TELEGRAM_REACTION_STYLE` | Which glyphs those reactions use: `lifecycle` (default — 👀, then 👍/👎) or `content` (👀, then a tone-matched glyph on purely social messages, plus the guarded reaction-only reply). Equivalent to `telegram.reaction_style`. ; unknown values fall back to `lifecycle`. |
